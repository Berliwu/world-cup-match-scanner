...
{
  "name": "wc2026-betting-value-scanner",
  "version": "1.0.0",
  "private": true,
  "type": "module",
  "description": "FIFA World Cup 2026 betting value scanner with Kelly sizing and market edge detection",
  "scripts": {
    "build": "tsc -p tsconfig.lib.json",
    "typecheck": "tsc -p tsconfig.json",
    "scan": "tsx src/cli/main.ts",
    "test": "vitest run",
    "test:watch": "vitest"
  },
  "devDependencies": {
...
# 22
cat > src/agent/valueScanner.ts <<'EOF'
import { GROUP_FIXTURES } from "../data/fixtures.js";
...
EOF
commit "feat: implement ValueScanner orchestrator class"
...
import { ValueScanner } from "../agent/predictionAgent.js";
import { getTeamById } from "../data/teams.js";
import { GROUP_FIXTURES } from "../data/fixtures.js";
...
async function main(): Promise<void> {
  const agent = new ValueScanner(true);

  switch (command) {
    case "predict": {
      const matchId = process.argv[3];
      if (!matchId) {
        logger.warn("Usage: npm run scan -- predict <matchId>");
        process.exit(1);
      }
...
export { ValueScanner } from "./agent/predictionAgent.js";
...
export const SYSTEM_PROMPT = `You are a FIFA World Cup 2026 value scanner.
...
mkdir -p src/betting
cat > src/betting/bankrollManager.ts <<'EOF'
...
EOF
commit "feat: add bankroll manager for WC2026 betting stakes"

cat > src/betting/edgeReport.ts <<'EOF'
...
EOF
commit "feat: add formatted edge report for value opportunities"

cat > src/betting/marketMargin.ts <<'EOF'
...
EOF
commit "feat: add bookmaker overround margin calculator"

cat > tests/bankrollManager.test.ts <<'EOF'
import { describe, it, expect } from "vitest";
...
EOF
commit "test: add bankroll manager unit tests"

mkdir -p src/config
cat > src/config/scannerThresholds.ts <<'EOF'
...
EOF
commit "feat: add WC2026 scanner edge and confidence thresholds"
...
```
EOF
commit "docs: add WC2026 betting value scanner README"
